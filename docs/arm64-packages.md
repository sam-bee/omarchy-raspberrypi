...
The TSV files are planning data, not an installer and not a claim that any package is currently available for aarch64. Every candidate and replacement still needs target-local repository/source resolution before a transaction is prepared. The replacement rows express package-name decisions for the ARM plan; they do not establish that the replacement has been built or tested on the Pi.

The initial candidate set is deliberately small: compositor/session and shell runtime, terminal, portals, fonts, clipboard, basic audio, file manager, package tooling, and SSH-friendly administration tools. Applications, developer toolchains, media production, indexing, printing, Docker, Bluetooth, optional services, and AUR/build workflows are deferred until the desktop is working and their ARM source and runtime behavior have been checked.
...
The third-party ARM branch was consulted only as historical evidence for package-name substitutions and omissions. Its installer and manifests are not the source of this policy. No network access, Pi access or package installation was used to produce it.
