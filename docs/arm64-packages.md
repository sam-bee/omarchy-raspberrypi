...
The TSV files are planning data, not an installer. A [read-only Pi package audit on 22 September 2026](arm64-package-audit.md) resolved the current candidate and replacement names against the Pi's existing sync databases and corrected four policy rows. A later [bounded first-session smoke](deployment/first-session/smoke-result.md) installed Hyprland, Mesa, Foot and their dependencies on the Pi; the broader candidate set remains a plan. Recheck target-local resolution and hooks for each further transaction. The replacement rows express package-name decisions for the ARM plan; they do not establish that every replacement has been runtime-tested on the Pi.
