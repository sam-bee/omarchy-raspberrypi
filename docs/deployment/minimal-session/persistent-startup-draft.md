# Persistent tty8 and RDP startup

This runbook records the original Sierra-specific deployment and acceptance. The source unit has since been replaced by the account-independent `omarchy-pi-uwsm-session@.service` template and a packaged RDP executable; follow the [Pi 5 minimal installation guide](../pi5-minimal-install.md) for new deployments. Paths and commands below describe the historical trial on the existing Pi.

**Status: manual lifecycle and controlled boot/reconnect gates passed on 23 September 2026; tty8 startup is enabled.** After the amended manual start/stop passed, a supervised reboot changed the Pi boot ID to `689027ee-0a2a-4f29-a5d3-989ff6786186`. The enabled system unit started the Sierra tty8 session, UWSM/Hyprland, and the user RDP service. A fresh encrypted recovery baseline `20260923T120330Z` was decrypted and independently compared with a GO result. Two actual FreeRDP clients connected after that boot; the second reconnected to the same compositor and server processes after the first client closed. See [persistent startup result](persistent-startup-result.md) for exact evidence and limits.

The earlier bounded FreeRDP run `20260923T111237Z` proved live view, keyboard and pointer input, reconnect, loopback-only listening, and bounded cleanup. Pi-native unit verification and the amended unit's manual start/stop trial passed. TLS certificate and key hashes stayed equal to the accepted manual-trial values, and the stop returned `Result=success`; the tty8 PAM session, graphical units, Wayland socket, and listener were gone, and tty1 was restored. The first stop attempt at `11:54:15 UTC` sent SIGHUP to UWSM before the then-configured `ExecStop`; UWSM cleaned up its graphical services, but the concurrent stop helper failed to create a PAM login because VT8 was already occupied and systemd marked the unit failed. That hook was removed. The successful retest used the `TTYVHangup=yes` SIGHUP path.

Files in `install/arm64/session/systemd/`:

- `omarchy-pi-uwsm-session.service` starts Sierra's PAM-backed tty8 session and the tested `start-hyprland` UWSM wrapper. Its privileged `chvt 8` pre-start was exercised in the manual trial. `TTYVHangup=yes` provides the stop path: it sends SIGHUP to UWSM, which stopped the graphical target and dependent services with `Result=success` in the amended manual retest. The unit was enabled after the recovery baseline passed and started successfully on reboot.
...
The enablement and reboot gate described here passed on 23 September 2026; see [persistent startup result](persistent-startup-result.md). The system tty8 unit is now enabled. The commands below record the gate used for this trial. Treat any later enablement change or reboot as a new controlled operation with a fresh baseline and recovery operator.
...
Evidence now includes the passing manual RDP run, Pi-native unit validation, amended manual lifecycle, controlled unattended boot, and postboot FreeRDP reconnect acceptance. The initial stop attempt cleaned up the session but failed its system unit result because the obsolete `ExecStop` helper raced the `TTYVHangup`-driven shutdown and could not create a second PAM login on tty8. The source and Pi helper were removed; the repeated start/stop passed with `Result=success`, TLS identity remained stable, and the later reboot and real-client checks passed. This establishes the tested boot/session/RDP path for this exact Pi state. It does not establish package-update safety: the available scratch full-upgrade preview had no candidates, and no package transaction was run.
